...
if(getRversion() >= "2.15.1") utils::globalVariables(c("cum_confirm", "cum_dead", 
    ...
    "percentage", "cum_heal", "confirmed", "map1"))
...
##' @param install_it If TRUE, the function will install packages needed
##' @import shiny
##' @import plotly
...
##' @export
open_dashboard <- function(lang="auto", remote=FALSE, install_it = FALSE) {

    if(! exists("map1")) {
        message("map1.Rdata is needed to draw city maps in the province. 
            You can download it from https://github.com/huerqiang/map-data and then load it")
    }
    ...
    if (remote) {
        ...
        mypy <- pinyin::pydic(method = "toneless",dic = 'pinyin2')    
        ...
        # ""武汉"" --> "Wuhan"
        {
          ...
          string <- as.character( pinyin::py(ChineseName, sep = "", dic = mypy) )
...
#              ,column(2, downloadButton('dataDownload', z('下载')	) )
            ...
            ,tags$head(includeScript(system.file("shinydata", "ga.js", package="nCov2019"))) # tracking usage with Google analytics      
        )           
        ...
        server <- function(input, output, session) {
            ...
            output$confirmedByProvincesHistorical <- renderPlot({
                    ...
                    p <- ggplot(d2,
                        ...
                        ggrepel::geom_text_repel(aes(label=province),  family="SimSun",data=d2[d2$time == time(x), ], hjust=1) +
            ...
            output$cities_in_proviences <- renderPlot({
                ...
                p <- ggplot(d,
                    ...
                    geom_point() + geom_line() +
                    ggrepel::geom_text_repel(aes(label=city), family="SimSun",data=d[d$time == time(x), ], hjust=1) +
                    theme_gray(base_size = 14) + theme(legend.position='none') +
                    xlab(NULL) + ylab(NULL) + 
            ...
            output$realTimeProvinceConfirmed <- renderPlot({
                ...
                d$name = forcats::fct_reorder(d$name, d$confirm)        
            ...
            output$realTimeCityConfirmedWorld <- renderPlot({
                d <- y['global',] %>%
                  ...
                  mutate (name = z2( name ) ) %>%
                  mutate( name = forcats::fct_reorder(name, confirm))
            ...
            output$historicalWorld <- renderPlotly({
              ...
              p <- ggplot(d,
                ...
                geom_point() + geom_line() +
                ggrepel::geom_text_repel(aes(label=country), data=d[d$time == time(x), ], hjust=1) +
            ...
            output$historicalChinaData <- renderPlotly({
                ...
                dl <- ChinaHistory %>%
                    tidyr::gather( type, count, c(confirm, heal, dead)) %>%
                ...
                p <- ggplot(dl,
                    ...
                    geom_point() + geom_line() +
                    ggrepel::geom_text_repel(aes(label=type), family="SimSun",data=dl[dl$time == time(x), ], hjust=1) +
                    theme_gray(base_size = 14) + #theme(legend.position='none') +
                    xlab(NULL) + ylab(NULL)  +
            ...
            output$historicalChinaDataAdd <- renderPlotly({
                ...
                dl <- pc %>%
                    tidyr::gather( type, percentage, c(confirm, heal, dead)) %>%
                    mutate( type = recode_factor(type,
                                                 confirm = z("确诊"),
                                                 dead = z("死亡"),
                                                 heal = z("痊愈"))) %>%
            ...
            output$provienceHistoricalAdd <- renderPlotly({
                ...
                dl <- d3 %>%
                    tidyr::gather( type, count, cum_confirm:cum_heal) %>%
                    mutate( type = recode_factor(type,
                                                 cum_confirm = z("确诊"),
                                                 cum_dead = z("死亡"),
                                                 cum_heal = z("痊愈")))
            ...
            output$cities_in_proviences_selected_plotly <- renderPlotly({
                ...
                dl <- d2 %>%
                    tidyr::gather( type, count, confirmed:heal) %>%
                    mutate( type = recode_factor(type,
                                                 confirmed = z("确诊"),
                                                 dead = z("死亡"),
                                                 heal = z("痊愈")))
            ...
            #世界 各国确诊人数预测, 预测-------------------------------------------    
              ...
              output$forecastConfirmedChangeWorld <- renderPlot ({
                ...
                graphics::par(mar = c(4, 3, 0, 2))
                # missing data with average of neighbors
                d2$confirm<- meanImput(d2$confirm, 2)
                
                confirm <- ts(d2$confirm, # percent change
                              start = c(lubridate::year(min(d2$time)), lubridate::yday(min(d2$time))  ), frequency=365  )
                forecasted <- forecast::forecast(forecast::ets(confirm), input$daysForcasted)
                graphics::plot(forecasted, xaxt="n", main="", 
                     ylab = z("全国确诊"),
                ...
                graphics::axis(1, at = lubridate::decimal_date(a), labels = format(a, "%b %d"))
              }, width = plotWidth - 100 ) 
            ...
            output$forecastConfirmedChange <- renderPlot ({
                ...
                confirm <- ts(diff(d2$confirm)/(10 + d2$confirm[1:(nrow(d2)-1)])*100, # percent change
                                start = c(lubridate::year(min(d2$time)), lubridate::yday(min(d2$time)) + 1 ), frequency=365  )
            ...
            output$forecastDeadChange <- renderPlot ({
                ...
                forecasted <- forecast::forecast(forecast::ets(dead), input$daysForcasted)
...
#                predictedNdeaded = d2$dead[nrow(d2)]* (1+ forecasted$mean[input$daysForcasted]/100)^input$daysForcasted 
            ...
            output$forecastDeadRaw <- renderPlot ({
                ...
                deaded <- ts(d2$dead, # percent change
                             start = c(lubridate::year(min(d2$time)), lubridate::yday(min(d2$time))  ), frequency=365  )
                forecasted <- forecast::forecast(forecast::ets(deaded), input$daysForcasted)
                graphics::plot(forecasted, xaxt="n", main="", 
                     ylab = z("全国死亡人数"),
            ...
            output$ChinaMap <- renderPlot ({
                ...
                graphics::plot(get_nCov2019(lang='en'), region='china', chinamap=cn,
                     continuous_scale=FALSE,
                     palette='Blues')
            ...
            output$provinceMap <- renderPlot ({
               ...
               # if(isEnglish | input$selectProvince0 %in% specialProvinces) { 
                if(isEnglish | !exists("map1")) { 
                    return(NULL)
                    } else {

                        graphics::plot(y, region = input$selectProvince0, 
                            chinamap = map1,
                            palette='Blues') 
