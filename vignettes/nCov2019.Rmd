...
ggplot(d, aes(time, count, color = curve)) +
  ...
  scale_x_date(date_labels = "%Y-%m-%d", 
    limits = c(as.Date("2020-01-15"), as.Date("2020-03-05"))) +
...
2. Outbreak Trend Curves of Countries Around the World (except China).
...
```{r globalwindrose, fig.width=20, fig.height=15}
require(nCov2019)
y <- load_nCov2019(lang = 'en', source='github')
d = y['global']
...
p + annotate(geom="text", label="Cases of Diamond Princess\nwas splited from Japan", x = 30, y = 400)
```
...
plot(y, region = 'china', continuous_scale = FALSE,
     chinamap = cn, date = '2020-02-01', font.size = 2)
```
